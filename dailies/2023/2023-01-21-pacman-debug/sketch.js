...
class Maze {
    ...
    drawPacman({gridX, gridY}) {
        let canvasPos = this.gridPosToCanvasPos({
            gridX: gridX,
            gridY: gridY
        });
        this.p.stroke(255);
        ...
        this.p.fill(this.pacmanColor);
        this.p.square(canvasPos[0], canvasPos[1], this.cellSize);
    }
     ...
     * Maze Helpers
     */
    gridPosToCanvasPos({gridX, gridY}) {
        let canvasX = this.x + (gridX * (this.cellSize + this.cellPadding));
        let canvasY = this.y + (gridY * (this.cellSize + this.cellPadding));
